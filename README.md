...
WorkBuddy（CodeBuddy）技能集合，分**场景型**与**角色型**两大类，支持按需远程安装。
...
| 分类 | 类型 | 目标用户 | 说明 |
|------|------|----------|------|
| `global/` | 场景 | **所有用户必装** | Agent 基础技能：自改进、技能生成、代码审查、规划管理、网页搜索 |
| `office/` | 场景 | 文档办公用户 | Word/Excel/PPT/PDF 处理、Obsidian、商业调研报告、提示词工程 |
| `search/` | 场景 | 研究者 | 深度调研工作流、多来源技能搜索引擎 |
| `custom/` | 场景 | 追求效率的用户 | 需求文档撰写、周报生成 |
| `roles/` | 角色 | 按职业分组 | 角色型技能集合，按职业细分，见下表 |

### `roles/` 角色子分类

| 子分类 | 目标用户 | 技能数 | 说明 |
|--------|----------|--------|------|
| `roles/developer/coding/` | 程序员 | 4 | GitHub 管理、代码交叉审查、全栈开发、笔记搜索 |
| `roles/designer/` | 前端开发者 | 1 | 生产级前端界面设计、避免 AI 通用风格 |
| `roles/ai-creation/` | AI 创作者 | 5 | AI 图片生成、语音转文字、视频下载、AI 资讯、旅行规划 |
| `roles/teacher/` | 教师用户 | 6 | 备课助手、试卷生成、成绩分析、报告评分、**代码评分** |
| `roles/hr/` | 人力资源 | 2 | 简历筛选、考勤统计 |
| `roles/software tester/` | 软件测试 | 1 | 自我排查与变通框架 |
| `roles/taobao-sku-monitor/` | 电商运营 | 1 | 淘宝竞品 SKU 价格监控 |
...
`docx` `xlsx` `pptx` `pdf` `pdfkit-py` `obsidian` `提示词工程专家` `业务调研报告撰写`
...
> `self-debug` 已移至 `roles/software tester/`

### roles/developer/coding/ — 4 个
...
### roles/ai-creation/ — 5 个

`AIHOT` `image-generation` `local-whisper` `yt-dlp-downloader` `携程问道`

### roles/teacher/ — 6 个

`teaching-preparation-assistant` `exam-paper-generator` `grade-analysis` `course-report-grading` `course-code-grading`

### roles/hr/ — 2 个

`resume-screening` `attendance-statistics`

### roles/software tester/ — 1 个

`self-debug`

### roles/taobao-sku-monitor/ — 1 个

`taobao-sku-monitor`

### archive/ — 2 个（已归档）

`Impeccable（前端设计工具集）` `agent-self-improvement`
...
| 用户表述 | 你的回应 |
|----------|----------|
| "装一下 WorkBuddy Skills" / 仅给了仓库链接，未指定分类 | **必须先问用户要装哪些分类**，列出 5 个分类让用户选择 |
...
# 安装单个技能
bash <(curl -sL https://raw.githubusercontent.com/bitcjm/workbuddy-skills/main/install.sh) --clone --skill roles/coding/github
```
...
bash install.sh roles/teacher          # 某个角色分类
bash install.sh --skill roles/coding/github  # 单个技能
bash install.sh --project global roles/coding  # 装到项目级
```

```powershell
.\install.ps1                          # 全量
.\install.ps1 global office            # 按分类
.\install.ps1 roles/teacher            # 某个角色分类
.\install.ps1 -Skill roles/coding/github   # 单个技能
.\install.ps1 -Project global roles/coding # 项目级
```
