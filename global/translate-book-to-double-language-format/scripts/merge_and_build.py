...
def generate_formats(temp_dir, lang_attr, cover=None, format_filter=None):
    """Generate DOCX, EPUB, and PDF with result summary.

    If format_filter is specified (e.g. '.epub'), only that format is generated.
    """
    ...
    if format_filter:
        ...
        if fmt in all_exts:
            exts = [fmt]
            print(f"Format filter: generating only {fmt}")
        else:
            print(f"Warning: unsupported format '{format_filter}', generating all formats")
            exts = all_exts
    else:
        exts = all_exts
...
def main():
    ...
    parser.add_argument('--format', default=None,
                        help='Only generate this format (e.g. .epub, .pdf, .docx). Default: all formats.')
    ...
    all_formats_ok = generate_formats(temp_dir, lang_cfg['lang_attr'], cover=cover, format_filter=args.format)
